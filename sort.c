...
#include <stdlib.h>
#define FIFTY 50
// Shifting i array elements right by one index from the position that the pointer holds.
...
// Returns the number (i) used for shift_elements().
...
void insertion_sort(int *arr, int len)
{
	...
	while (ptr2 != end) // if my Second pointer : ptr2 is pointing the last element of my array i need to step out
	{
		int counter = 0;
		if (*ptr2 < *ptr1)
		{											// if : the value of ptr2 < value of (ptr2-1) i gonna need to do swaping
...
int nums[FIFTY];
// Getting the numbers from terminal STREAM.
void getNumbers()
{
